...
namespace gj{
    ...
    class processor{
        ...
        public:
        unordered_set<std::string> project_files;
        processor(char ** compilerOptionsStart, char ** compilerOptionsEnd, const std::string & cacheDir = DEFAULT_CACHE_DIR);
        virtual ~processor();
        pair<global_hint_base_t, vector<string> > collect(const std::string & filename);
        void recache(const std::string & filename);
        vector<hint_t> resolve_position(const std::string & filename, pos_t pos);
        vector<hint_t> resolve_position(const std::string & filename, int line, int index);
        void full_recache();
    };
